...
public class BackwardsPark extends CommandBase {
   ...
   */
  public BackwardsPark(SwerveDrivetrain swerve) {
    this.swerve = swerve;
    rotation = new Rotation2d(0);
    backward = new SwerveModuleState(-0.7,rotation);
  ...
  public boolean isFinished() {
    if(Math.abs(swerve.getGyroRoll()) >6){
      return false;
    }
    else{
      return true;
